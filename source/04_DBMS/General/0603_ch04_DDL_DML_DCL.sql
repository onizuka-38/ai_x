...
--서브쿼리를 이용한 INSERT
	-- EX. DEPT테이블에서 10~30부서의 내용을 DEPT01 테이블에 INSERT
...
CREATE TABLE EMP01 
	AS SELECT EMPNO, ENAME, SAL, DEPTNO FROM EMP; -- 테이블내용 복제(제약조건은 복제 안 됨)
	-- 부서번호를 30으로 수정
...
-- (2) FOREIGN KEY : 테이블의 열이 다른 테이블의 열을 참조하는 경우
-- (3) NOT NULL	 : NULL 을 포함하지 않는 필드
-- (4) UNIQUE : 모든 행의 값이 유일한 값. NULL은 허용(NULL은 여러개 입력 가능)
-- (5) CHECK(조건) : 해당 조건이 만족(NULL 값 허용)
-- DEFAULT 기본값 : 해당 열의 데이터를 입력하지 않으면 원래는 NULL 이 들어갈 것을 기본값이 들어가도록

-- DEPT1 & EMP1 (exERD 를 이용하여 ERD 작성 =테이블설계 => )
DROP TABLE DEPT1;
CREATE TABLE DEPT1(
	DEPTNO	 NUMBER(2) PRIMARY KEY,
...
INSERT INTO EMP1 (EMPNO, ENAME, JOB) VALUES (1001, 'HONGGILDONG', 'MANAGER'); -- 오류
